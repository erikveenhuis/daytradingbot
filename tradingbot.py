...
class CustomAgent:
    ...
    def __init__(self, lookback_window_size=60, lr=0.00005, epochs=1, optimizer=Adam, batch_size=32, model="", depth=0, comment=""):
        ...
        self.log_name = datetime.now().strftime("%Y_%m_%d_%H_%M")+"_momentum_trader"
        
        # State size contains Market+Orders+Indicators history for the last lookback_window_size steps
        self.state_size = (lookback_window_size, 18) # 5 standard OHCL information + market and indicators (5+5+8)
    ...
    def save(self, name="Momentum_trader", score="", args=[]):
...
class CustomEnv:
    # A custom momentum trading environment
    def __init__(self, mode, initial_balance=1000, lookback_window_size=100, Render_range=100, Show_reward=True, Show_indicators=True, normalize_value=10000):
        ...
        # Orders history contains the balance, net_worth, shares_bought, shares_sold, shares_held values for the last lookback_window_size steps
    ...
    def reset(self, env_steps_size = 0):
        ...
        df, df_normalized = getDfAndDfNormalized(self.mode)
        ...
        for i in reversed(range(self.lookback_window_size)):
            ...
            self.orders_history.append([self.balance / self.normalize_value,
                                        ...
                                        self.shares_bought / self.normalize_value,
                                        self.shares_sold / self.normalize_value,
                                        self.shares_held / self.normalize_value
                                        ])
    ...
    def step(self, action):
        ...
        if action == 0: # Hold
            ...
            self.shares_bought = self.balance / current_price
            self.shares_bought *= (1-self.fees) # substract fees
            self.balance -= self.shares_bought * current_price
            self.shares_held += self.shares_bought
            self.trades.append({'Date' : Date, 'High' : High, 'Low' : Low, 'total': self.shares_bought, 'type': "buy", 'current_price': current_price})
            self.episode_orders += 1

        elif action == 2 and self.shares_held*current_price> self.initial_balance*0.05:
            # Sell 100% of current shares held
            self.shares_sold = self.shares_held
            self.shares_sold *= (1-self.fees) # substract fees
            self.balance += self.shares_sold * current_price
            self.shares_held -= self.shares_sold
            self.trades.append({'Date' : Date, 'High' : High, 'Low' : Low, 'total': self.shares_sold, 'type': "sell", 'current_price': current_price})
            self.episode_orders += 1
...
def train_agent(env, agent, visualize=True, train_episodes = 50, training_batch_size=500):
...
def _test_agent(env, agent, visualize=True, test_episodes=10, folder="", name="Momentum_trader", comment=""):
...
def test_agent(test_df, test_df_nomalized, visualize=True, test_episodes=10, folder="", name="", comment="", Show_reward=True, Show_indicators=True):
    ...
    agent = CustomAgent(lookback_window_size=params["lookback window size"], optimizer=Adam, model=params["model"])
...
def getDfAndDfNormalized(mode):
    if mode == 'train':
        folder = './train/'
    elif mode == 'test':
        folder = './test/'
    else:
        Exception('Use mode train or test')
    ...
    #df = indicators_dataframe(df, threshold=0.5, plot=False) # insert indicators
    # depth = len(list(df.columns[1:])) # OHCL + indicators without Date

    df_normalized = Normalizing(df[99:])[1:].dropna()
    df = df[100:].dropna()
...
if __name__ == "__main__":            
    ...
    # train_env = CustomEnv(mode=mode, lookback_window_size=lookback_window_size)
    # train_agent(train_env, agent, visualize=True, train_episodes=50000, training_batch_size=500)
    
    # multiprocessing training/testing. Note - run from cmd or terminal
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=5, optimizer=Adam, batch_size=32, model="CNN", comment="Train")
    train_multiprocessing(CustomEnv, agent, mode, num_worker = 32, training_batch_size=500, visualize=False, EPISODES=500000)

    # test_multiprocessing(CustomEnv, CustomAgent, mode='test', num_worker = 10, visualize=False, test_episodes=1000, folder="2021_04_18_14_56_momentum_trader", name="1013.74_Momentum_trader", comment="Test")