...
class TradingGraph:
    ...
    def __init__(self, Render_range, Show_reward=True, Show_indicators=True):
    ...
    def render(self, df, net_worth, trades):
        ...
        self.ax1.clear()
        candlestick_ohlc(self.ax1, self.render_data, width=0.000009, colorup='green', colordown='red', alpha=0.75)
        ...
        cv2.imshow("Momentum trading bot",image)
...
def Plot_OHCL(df):
    ...
    # beautify the x-labels (Our Date format)
    ax1.xaxis.set_major_formatter(mpl_dates.DateFormatter('%y-%m-%d %H:%M:%S'))# %H:%M:%S'))
    fig.autofmt_xdate()
    fig.tight_layout()
